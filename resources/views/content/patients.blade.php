        ...
        <div class="flex items-center justify-between ">
            <label class="flex items-center gap-2" for="time">
                ...
                <form method="GET" action="{{ route('patient_list') }}" class="flex gap-4 items-center justify-center m-4">
                    <h1>Sort by: </h1>
                    <select name="sort" class="border text-sm w-32 border-gray-400 p-2 px-4 mx-2 rounded-md" id="sort">
                        <option value="id" {{ request()->get('sort') == 'id' ? 'selected' : '' }}>ID</option>
                        <option value="date_of_next_visit" {{ request()->get('sort') == 'date_of_next_visit' ? 'selected' : '' }}>Next visit</option>
                        <option value="name" {{ request()->get('sort') == 'name' ? 'selected' : '' }}>Name</option>
                        <option value="package" {{ request()->get('sort') == 'package' ? 'selected' : '' }}>Package</option>
                    </select>
                </form>
            </label>
                    ...
                    <tr class="odd:bg-green-100 even:bg-slate-100">
                        ...
                        <td class="border px-4 py-2">
                            <div class="flex justify-center items-center gap-6">
                                ...
                                <div class="tooltip">
                                    <img class="h-8" src="{{ asset('assets/images/facebook.png') }}" alt="Facebook Icon">
